...
def calc_hanning(m: int, n: int) -> np.ndarray:
    """Calculates the first m points of an n point hanning window."""
    window = 0.5 * (1 - np.cos(2 * np.pi * np.arange(1, m + 1) / (n + 1)))
    return window


def make_gauss_hanning(flip_angle: float, pulse_duration: float, system: Opts = Opts()) -> SimpleNamespace:
    """Creates an RF pulse event for a gaussian pulse with hanning window."""
